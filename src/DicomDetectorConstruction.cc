...
G4VPhysicalVolume* DicomDetectorConstruction::Construct()
{
  if(!fConstructed || fWorld_phys == 0) {
    ...
    //----- Build world
    G4double worldXDimension = 1.5*m;
    G4double worldYDimension = 1.5*m;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double DicomDetectorConstruction::GetVoxelMass(G4int x, G4int y, G4int z) const
{
        G4int VoxelNumber = x + y*fNVoxelX + z*fNVoxelX*fNVoxelY;
	G4int MaterialID=fMateIDs[VoxelNumber];
	G4Material* VoxelMaterial = fMaterials[MaterialID];
	G4double Density=VoxelMaterial->GetDensity();
	G4double Volume=fVoxelHalfDimX*2*fVoxelHalfDimY*2*fVoxelHalfDimZ*2;
